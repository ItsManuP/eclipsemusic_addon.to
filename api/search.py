...
async def add_torrent_to_torbox(magnet: str, api_token: str):
    """Aggiunge un torrent a TorBox e restituisce il torrent_id."""
    async with httpx.AsyncClient(timeout=10.0) as client:
        headers = {"Authorization": f"Bearer {api_token}"}
        # Usa data= per form-urlencoded (come da documentazione)
        resp = await client.post(
            f"{TORBOX_API_URL}/torrents/createtorrent",
            data={"magnet": magnet},
            headers=headers
        )
        # Se la risposta non è 200, solleva errore con il testo
        if resp.status_code != 200:
            raise Exception(f"HTTP {resp.status_code}: {resp.text}")
        
        result = resp.json()
        # Controlla il campo 'success'
        if not result.get("success"):
            raise Exception(f"TorBox API error: {result.get('error')} - {result.get('detail')}")
        
        # Estrae l'ID: la struttura potrebbe essere diversa
        ...
        if not torrent_id:
            # Se non trovato, mostra la struttura per il debug
            raise Exception(f"Campo 'id' non trovato in data: {data}")
...
async def get_torrent_status(torrent_id: str, api_token: str):
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        # Ottieni il link di streaming (requestdl restituisce un redirect)
...
async def search_endpoint(q: str):
    ...
    try:
        ...
        # Restituiamo l'errore completo (utile per debug)
...
@app.get("/")
async def root():
    return {"status": "ok", "message": "Eclipse Torrent Addon (async TorBox with debug)"}
